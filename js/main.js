...
(function () {
  ...
  // ----- Galeria: move setas pra dentro do stage no desktop, mantém embaixo no mobile -----
  (function setupGalleryArrowsLayout() {
    const stage = document.querySelector('.galeria-editorial__stage');
    const nav = document.querySelector('.galeria-editorial__nav');
    const prevBtn = document.querySelector('[data-gallery-prev]');
    const nextBtn = document.querySelector('[data-gallery-next]');
    if (!stage || !nav || !prevBtn || !nextBtn) return;

    const mq = window.matchMedia('(min-width: 761px)');
    let currentMode = null;

    function applyLayout() {
      const targetMode = mq.matches ? 'overlay' : 'bottom';
      if (currentMode === targetMode) return;

      if (mq.matches) {
        // Desktop: setas dentro do stage (overlay nas laterais da foto)
        stage.appendChild(prevBtn);
        stage.appendChild(nextBtn);
      } else {
        // Mobile: setas no nav inferior, prev antes dos dots, next depois
    ...
    if (mq.addEventListener) {
      ...
      mq.addListener(applyLayout); // Safari antigo
    }
  })();
})();
